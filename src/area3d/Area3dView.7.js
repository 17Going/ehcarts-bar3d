...
function createArea(wrap, data, coordSys, seriesModel, isHorizontal) {
    ...
    // var len = upPoints.length;
    // upPoints[0] = [upPoints[0][0] + faceWidth, upPoints[0][1]];
    // upPoints[len - 1] = [upPoints[len - 1][0] + faceWidth, upPoints[len - 1][1]];
    // 获取坐标轴上的坐标点
    ...
    // upStackedOnPoints[0] = [upStackedOnPoints[0][0] + faceWidth, upStackedOnPoints[0][1]];
    // upStackedOnPoints[len - 1] = [upStackedOnPoints[len - 1][0] + faceWidth, upStackedOnPoints[len - 1][1]];
    ...
    // 绘制顶面
    var topFace;
    if (!(topFace = g.childOfName(TOP_FACE))) {
        var topG = new graphic.Group();
        topG.name = TOP_FACE;
        for (var i = 0; i < len - 1; i++) {
            var circle = new _poly.Polygon1({
                shape: {
                    i: i,
                    len: i + 2,
                    points: points,
        ...
        topG.setShape = function () {

        }

        topG.useStyle = function (style) {
            topG.eachChild(function (item) {
                item.useStyle(style);
            });
        }
    ...
    var topFace1 = g.childOfName(TOP_FACE) ? g.childOfName(TOP_FACE) :
        new Polygon({
            name: 'face',
            z2: 10
        });
    ...
    // 绘制右侧面
    var leftFace = g.childOfName(LEFT_FACE) ? g.childOfName(LEFT_FACE) :
        new Polygon({
            name: LEFT_FACE,
            z2: 5
        });
    ...
    // 绘制左侧面
    var rightFace = g.childOfName(RIGHT_FACE) ? g.childOfName(RIGHT_FACE) :
        new Polygon({
            name: RIGHT_FACE,
            z2: 5
        });
    ...
    //绘制正面
    var frontFace = g.childOfName(FRONT_FACE) ? g.childOfName(FRONT_FACE) :
        new Polygon({
            name: FRONT_FACE,
            shape: {
                points: points,
    ...
    // 绘制背面
    var backFace = g.childOfName(BACK_FACE) ? g.childOfName(BACK_FACE) :
        new Polygon({
            name: BACK_FACE,
            shape: {
                points: upPoints,
 ...
 * @param {Boolean} isHorizontal 
 */
function getPoint(point, prePoint, faceWidth, isHorizontal) {
...
function updateStyle(g, data, coordSys, itemModel) {
    ...
    g.childOfName(TOP_FACE).useStyle(zrUtil.defaults({ opacity: 0.25, fill: topFaceColor }, defaultsStyle));
    g.childOfName('face').useStyle(zrUtil.defaults({ opacity: 0.25, fill: topFaceColor }, defaultsStyle));
